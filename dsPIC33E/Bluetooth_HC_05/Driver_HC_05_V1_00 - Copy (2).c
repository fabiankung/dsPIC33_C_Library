...
//
//  (c) Copyright 2016-2018, Fabian Kung Wai Lee, Selangor, MALAYSIA
//  All Rights Reserved
//
//////////////////////////////////////////////////////////////////////////////////////////////
//
...
// Toolsuites		: Microchip MPLAB X IDE v4.05 or above
...
///
/// DRIVER		:  Driver_Serial_Comm_V100
///
...
void Proce_BT_HC_05_CommStack(TASK_ATTRIBUTE *ptrTask)
{
        ...
        switch (ptrTask->nState)
	{
            case 0: // State 0 - Initialization of HC-05 Bluetooth module.
                ...
                OSSetTaskContext(ptrTask, 1, 500*__NUM_SYSTEMTICK_MSEC); // Next state = 1,
                                                    // timer = 500msec. A sufficient long
            ...
            case 2: // State 2 - Reset HC-05 bluetooth module.
                ...
                OSSetTaskContext(ptrTask, 3, 20*__NUM_SYSTEMTICK_MSEC);     // Next to state = 3, timer = 20msec.
                break;
            ...
            case 3: // State 3 - Deassert the reset pin.
                ...
                if (gnEnHC05Init == 1)
                {   
                    OSSetTaskContext(ptrTask, 4, 1000*__NUM_SYSTEMTICK_MSEC);     // Next to state = 4, timer = 1sec.
                }
                else
                {
            ...
            case 7: // State 7 - Wait until all data is send.
             ...
             case 8: // State 8 - De-assert AT command mode and set new UART1 baud rate.
                ...
                U1BRG = (__FOSC_MHz/8)*(1000/_BT_UART_BAUDRATE_kBPS)-1;         // Update new baud rate for internal
                                                                                // UART module.
                OSSetTaskContext(ptrTask, 9, 200*__NUM_SYSTEMTICK_MSEC);        // Next to state = 9, timer = 0.2sec.
                break;

            case 9: // State 9 - Clear UART receive buffer and proceed.
            ...
            case 11: // State 11 - De-assert reset, entering into Communication mode.
                PIN_HC_05_RESET = 1;                     // De-assert reset pin.
                OSSetTaskContext(ptrTask, 12, 1*__NUM_SYSTEMTICK_MSEC);     // Next to state = 12, timer = 1 msec.
                break;
                
            // --- Specifics sequence to establish SPP (serial port protocol) with remote host ---
            case 12: // State 12 - Wait for initial message from host computer to establish SPP link.
                ...
                if ((gSCIstatus.bRXRDY == 1) && (gbytRXbufptr > 4))     // Check if UART receives any valid data.
                {                                                       // data packet from remote host.
                    if (gSCIstatus.bRXOVF == 0)                         // Make sure no overflow error.
                    {
                        if ((gbytRXbuffer[0] == gobjDriverHC05.bytRFAdd)&&(gbytRXbuffer[1] == 0x55) && (gbytRXbuffer[2] == _SPP_ESTABLISH_LINK))
                        {
                            ...
                            OSSetTaskContext(ptrTask, 13, 100);         // Next state = 13, timer = 100.
                        }
                        else
                        {
                            OSSetTaskContext(ptrTask, 12, 100);         // Next state = 12, timer = 100.
                        }
                    }
                    else
                    {
            ...
            case 13: // State 13 - Reply host computer, note that standard packet consist
                    // of 8 bytes.
                    // Byte 0 = Robot RF Address.
                    // Byte 1 = 0x55 (Command)
