...
//////////////////////////////////////////////////////////////////////////////////////////////
//
// File			: Drivers_Dynamixel_Motor_V100.c
...
BYTE        gbytInstruction_Array[15];          // Array to hold instruction packet data
BYTE        gbytStatus_Array[8];		// Array to hold returned status packet data
long        glngTime_Counter;                   // Timer for time out watchers
BYTE        gbytStatus_Return_Value = 0;	// Status packet return states ( NON , READ , ALL )
...
#define	_UART_BAUDRATE_kBPS	57.6	// Default datarate in kilobits-per-second, for dynamixel motor.

///
/// Process name	: Proce_Dynamixel_Motor_Driver
///
/// Author		: Fabian Kung
///
/// Last modified	: 23 March 2015
...
///  			  2. Pin RG7 = U1RX (Remappable I/O RPI119), input.
...
void Proce_Dynamixel_Motor_Driver(TASK_ATTRIBUTE *ptrTask)
{
	...
	if (ptrTask->nTimer == 0)
	{
		switch (ptrTask->nState)
		{
			case 0: // State 0 - UART1 Initialization.
				...
				U1STAbits.UTXEN = 1; // Enable Transmitt module in UART.
			...
			case 1: // State 1 - Transmitt and receive buffer manager.
				...
				{
                                    while (U1STAbits.UTXBF == 0)                // Check if UART transmitt buffer is not full.
										// 2 conditions to exit the load transmit buffer
                                                                                // routines, (1) When transmit butter is or
					...
					{
                                            U1TXREG = gbytTXbuffer[gbytTXbufptr];  // Load 1 byte data to UART transmitt buffer.
					    ...
					    gSCIstatus.bTXRDY = 0;              // Reset transmitt flag.
				...
				// Note that the receive FIFO buffer is 4-level deep in dsPIC33 and PIC24H microcontrollers.
                                ...
                                {
                                    U1STAbits.OERR = 1;                         // Clear overrun flag, this will also
